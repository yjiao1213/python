...
def compute_a(z):
    ...
    a = []
    for item in z:
        a.append(math.pow(math.e, item - max_value))
    sum_a = sum(a)
...
def compute_da_dz(a):
    ...
    da_dz = np.mat(np.zeros((len(a), len(a))))

    for i in range(np.shape(da_dz)[0]):
        for j in range(np.shape(da_dz)[1]):
            if i == j:
                da_dz[i, j] = a[i]  - a[i] ** 2
            else:
                da_dz[i, j] = - a[i] * a[j]
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    dL_dz_inst = np.tile(dL_dz.T, (np.shape(dz_dW)[1],1))
    dL_dW = np.multiply(dL_dz_inst.T, dz_dW)
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        y_y = W * x + b
        sum_y = np.sum(y_y)
        y_label = np.where(y_y == np.max(y_y))[0]
        Y[i] = y_label
        y_y = compute_a(y_y)
        for j in range(y_y.shape[0]):
            P[i, j] = y_y[j]
        #########################################
    return Y, P 
