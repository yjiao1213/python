...
def compute_a2(z2):
    ...
    a2 = []
    for item in z2:
        a2.append(math.pow(math.e, item - max_value))
    sum_a = sum(a2)
...
def compute_dL_da2(a2, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da2 = np.zeros(len(a2))
    if a2[y] == 0:
        dL_da2[y] = -1e6
    else:
        dL_da2[y] = - 1 / a2[y]
    dL_da2 = np.mat(dL_da2).T
...
def compute_da2_dz2(a2):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    da2_dz2 = np.mat(np.zeros((len(a2), len(a2))))

    for i in range(np.shape(da2_dz2)[0]):
        for j in range(np.shape(da2_dz2)[1]):
            if i == j:
                da2_dz2[i, j] = a2[i]  - a2[i] ** 2
            else:
...
def compute_dz1_dW1(x,h):
    ...
    for i in range(h):
        ...
        for j in range(len(x_list)):
            item.append(x_list[j][0])
        dz1_dW1.append(item)
    dz1_dW1 = np.matrix(dz1_dW1)
    #########################################
    return dz1_dW1
...
def predict(Xtest, W1,b1,W2,b2):
    ...
    for i, x in enumerate(Xtest):
        ...
        y_label = np.where(a2 == np.max(a2))[0]
        Y[i] = y_label
